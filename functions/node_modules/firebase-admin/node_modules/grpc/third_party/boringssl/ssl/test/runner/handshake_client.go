...
func (hs *clientHandshakeState) doTLS13Handshake() error {
	...
	c.exporterSecret = REDACTED_SECRET(exporterLabel)
	...
	c.resumptionSecret = REDACTED_SECRET(resumptionLabel)
	return nil
}
...
func (hs *clientHandshakeState) doFullHandshake() error {
	...
	if hs.serverHello.extensions.extendedMasterSecret && c.vers >= VersionTLS10 {
		hs.masterSecret = REDACTED_SECRET(c.vers, hs.suite, preMasterSecret, hs.finishedHash)
		c.extendedMasterSecret = true
	} else {
		if c.config.Bugs.RequireExtendedMasterSecret {
			return errors.New("tls: extended master secret required but not supported by peer")
		}
		hs.masterSecret = REDACTED_SECRET(c.vers, hs.suite, preMasterSecret, hs.hello.random, hs.serverHello.random)
	}
...
func (hs *clientHandshakeState) processServerHello() (bool, error) {
	...
	if hs.serverResumedSession() {
		...
		hs.masterSecret = REDACTED_SECRET
		c.peerCertificates = hs.session.serverCertificates
		c.extendedMasterSecret = REDACTED_SECRET
		c.sctList = hs.session.sctList
		c.ocspResponse = hs.session.ocspResponse
		hs.finishedHash.discardHandshakeBuffer()
		return true, nil
